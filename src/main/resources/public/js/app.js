...
//new functions =================================================================================
function startGame() {
    if(gameModel.aircraftCarrier.start.Across != 0
        && gameModel.clipper.start.Across != 0
        && gameModel.battleship.start.Across != 0
        && gameModel.dinghy.start.Across != 0
        && gameModel.submarine.start.Across != 0) {
        var request = $.ajax({
            url: "/startGame/"+$( "#diffSelec" ).val(),
            method: "post",
        ...
        alert("Finish placing your ships!");
    }
}
...
function fire(){
    if(gameModel.gameInProgress) {
        ...
        //var menuId = $( "ul.nav" ).first().attr( "id" );
        var request = $.ajax({
            url: "/fire/"+$( "#colFire" ).val()+"/"+$( "#rowFire" ).val()+"/"+$( "#diffSelec" ).val(),
            method: "post",
            data: JSON.stringify(gameModel),
            contentType: "application/json; charset=utf-8",
            dataType: "json"
        ...
        request.done(function( currModel ) {
            displayGameState(currModel);
            gameModel = currModel;
            if(gameModel.gameInProgress == false) {
                if(gameModel.playerWon == true)
                    alert("The player won! please reload the page.");
                else
                    alert("The player lost. Please reload the page sadly.");
            }
        });
       ...
       request.fail(function( jqXHR, textStatus ) {
        ...
        gameModel.errorMessage = "The game has not started";
        alert(gameModel.errorMessage);
        gameModel.errorMessage = "none";
    }
}
...
function scan(){
    if(gameModel.gameInProgress) {
        ...
        //var menuId = $( "ul.nav" ).first().attr( "id" );
        var request = $.ajax({
             url: "/scan/"+$( "#colScan" ).val()+"/"+$( "#rowScan" ).val()+"/"+$( "#diffSelec" ).val(),
            method: "post",
            data: JSON.stringify(gameModel),
            contentType: "application/json; charset=utf-8",
            dataType: "json"
